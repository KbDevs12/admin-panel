        ...
        <Card key={stat.label}>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <stat.icon
                className={`size-4 ${stat.highlight ? "text-destructive" : "text-muted-foreground"}`}
              />
            </div>
          </CardHeader>
          <CardContent className="space-y-1">
            <p
              className={`text-2xl font-semibold tabular-nums ${stat.highlight ? "text-destructive" : "text-foreground"}`}
            >
              {stat.value}
            </p>
